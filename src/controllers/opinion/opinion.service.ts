...
export class OpinionService {
  ...
  async addTags(tag: any, opinion_id: any) {
    if (tag.id && !(await this.tagRepository.findTag(tag.id)))
      return "not existent id";
    let tagByName = await this.tagRepository.findTag(tag.name.toUpperCase());
    if (tag.name && tagByName)
      tag.id = (await this.tagRepository.findTag(tag.name.toUpperCase())).id;
    if (!tag.id) {
